...
?>
    ...
    <section class="content">
      ...
      <div class="modal fade" id="modal-default-add">
        <div class="modal-dialog">
          <div class="modal-content">
            <form id="AddForm" action="data/subcategory_add_data.php" method="post">
              ...
              <div class="modal-body">
				...
				<div class="form-group">
					...
					<input id="addSubCategory" name="addSubCategory" class="form-control" type="text" placeholder="Enter sub category name" required>
				</div>
              </div>
      ...
      <div class="modal fade" id="modal-default-update">
        <div class="modal-dialog">
          <div class="modal-content">
            <form id="UpdateForm" action="data/subcategory_update_data.php" method="post">
              <div class="modal-header">
              ...
              <div class="modal-body">
				...
				<div class="form-group">
					<label>Sub Category <span class="req">*</span></label>
					<input id="updateSubCategory" name="updateSubCategory" class="form-control" type="text" placeholder="Update sub category name" required>
...
$(document).ready(function() {
	...
	//--- Load data to table ---//
	function loadData(){
		$.post( "data/subcategory_load_data.php", function(result,status){
			table.clear();
	...
	$("#AddForm").submit(function(){
		...
		$.post( $("#AddForm").attr("action"), $("#AddForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Sub Category successfully saved!");
			}
			else {
	...
	$(document).delegate(".updateItem", "click", function() {
			...
			$.post( "data/subcategory_get_data.php", { itemID: dataID }, function(result){
				var obj = JSON.parse(result);
				$("#updateCategory").val(obj[0].categoryID);
				$("#updateSubCategory").val(obj[0].name);
				$("#updateID").val(obj[0].subCategoryID);
			});
	...
	$("#UpdateForm").submit(function(){
		...
		$.post( $("#UpdateForm").attr("action"), $("#UpdateForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Sub Category successfully updated!");
			}
			else {
				errorNotifNoload(result);
			}
	...
	$(document).delegate(".deleteItem", "click", function() {
		...
		if (confirm("Are you sure you want to delete this record?")) {
		    var dataID = $(this).attr('data-id'); //get the item ID
			$.post( "data/common_delete_data.php", { tblName: "tbl_subcategory", fieldName: "subCategoryID", notifName: "Sub Category", itemID: dataID }, function(result,status){
				if (result == 'Success'){
					loadData();
					successNotifNoload("Sub Category successfully deleted!");
				} 
				else {
					errorNotifNoload(result);
				}
