          ...
          <div class="modal-content">
            <form id="AddForm" action="data/common_add_data.php" method="post">
              ...
              <div class="modal-body">
                <input id="addinputName" name="addinputName" class="form-control" type="text" placeholder="Enter unit of measure name" required>
				<input type="hidden" id="addtblName" name="addtblName" value="tbl_uom">
				<input type="hidden" id="addfieldName" name="addfieldName" value="name">
				<input type="hidden" id="addnotifName" name="addnotifName" value="Unit of Measure">
              </div>
              <div class="modal-footer justify-content-between">
                <button type="button" id="closeAddForm" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-primary">Add Unit of Measure</button>
              </div>
            </form>
          </div>
          <!-- /.modal-content -->
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <form id="UpdateForm" action="data/common_update_data.php" method="post">
              <div class="modal-header">
                <h4 class="modal-title">Update Unit of Measure</h4>
                <button type="button" id="closeUpdateFormX" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <input id="updateinputName" name="updateinputName" class="form-control" type="text" placeholder="Update unit of measure name" required>
				<input type="hidden" id="updatetblName" name="updatetblName" value="tbl_uom">
				<input type="hidden" id="updatefieldID" name="updatefieldID" value="uomID">
				<input type="hidden" id="updatefieldName" name="updatefieldName" value="name">
				<input type="hidden" id="updatenotifName" name="updatenotifName" value="Unit of Measure">
...
$(document).ready(function() {
		...
		$.post( "data/common_load_data.php", { tblName: "tbl_uom", sortName: "name" }, function(result,status){
			...
			var obj = JSON.parse(result);
			 obj.forEach(function(item) {
				table.row.add([item.name, "<div class='btn-group btn-group-sm'><a href='#' class='btn btn-warning updateItem' data-id="+item.uomID+" data-name="+item.name+" data-toggle='modal' data-target='#modal-default-update' title='Update'><i class='fas fa-pen'></i></a><a href='#' class='btn btn-danger deleteItem' data-id="+item.uomID+" title='Delete'><i class='fas fa-trash'></i></a></div>"])
			})
			table.draw(false);
		});
	}
	...
	$("#AddForm").submit(function(){
		...
		$.post( $("#AddForm").attr("action"), $("#AddForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Unit of Measure successfully saved!");
			}
			else {
	...
	$("#UpdateForm").submit(function(){
		...
		$.post( $("#UpdateForm").attr("action"), $("#UpdateForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Unit of Measure successfully updated!");
			}
			else {
				errorNotifNoload(result);
			}
	...
	$(document).delegate(".deleteItem", "click", function() {
		...
		if (confirm("Are you sure you want to delete this record?")) {
		    var dataID = $(this).attr('data-id'); //get the item ID
			$.post( "data/common_delete_data.php", { tblName: "tbl_uom", fieldName: "uomID", notifName: "Unit of Measure", itemID: dataID }, function(result,status){
				if (result == 'Success'){
					loadData();
					successNotifNoload("Unit of Measure successfully deleted!");
				} 
				else {
					errorNotifNoload(result);
				}
