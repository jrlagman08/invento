<?php
	...
	if(isset($_POST["isRepackage"]) == 1) {
		$sql = "SELECT prod.*,cat.name as cat,class.name as clas,clr.name as clr,ssn.name as ssn,uom.name as uom,wrh.name as wrh,rak.name as rak 
		...
		WHERE prod.isRepackage = 1 ORDER BY prod.prodName ASC";
	} else {
		$sql = "SELECT prod.*,cat.name as cat,class.name as clas,clr.name as clr,ssn.name as ssn,uom.name as uom,wrh.name as wrh,rak.name as rak 
		FROM tbl_product prod 
		INNER JOIN tbl_category cat ON prod.categoryID=cat.categoryID 
