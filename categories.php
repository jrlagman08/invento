          ...
          <div class="modal-content">
            <form id="AddForm" action="data/common_add_data.php" method="post">
              ...
              <div class="modal-body">
                <input id="addinputName" name="addinputName" class="form-control" type="text" placeholder="Enter category name" required>
				<input type="hidden" id="addtblName" name="addtblName" value="tbl_category">
				<input type="hidden" id="addfieldName" name="addfieldName" value="name">
				<input type="hidden" id="addnotifName" name="addnotifName" value="Category">
              </div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <form id="UpdateForm" action="data/common_update_data.php" method="post">
              ...
              <div class="modal-body">
                <input id="updateinputName" name="updateinputName" class="form-control" type="text" placeholder="Update category name" required>
				<input type="hidden" id="updatetblName" name="updatetblName" value="tbl_category">
				<input type="hidden" id="updatefieldID" name="updatefieldID" value="categoryID">
				<input type="hidden" id="updatefieldName" name="updatefieldName" value="name">
				<input type="hidden" id="updatenotifName" name="updatenotifName" value="Category">
		...
		$.post( "data/common_load_data.php", { tblName: "tbl_category", sortName: "name" }, function(result,status){
			...
			var obj = JSON.parse(result);
			 obj.forEach(function(item) {
				table.row.add([item.name, "<div class='btn-group btn-group-sm'><a href='#' class='btn btn-warning updateItem' data-id="+item.categoryID+" data-name="+item.name+" data-toggle='modal' data-target='#modal-default-update' title='Update'><i class='fas fa-pen'></i></a><a href='#' class='btn btn-danger deleteItem' data-id="+item.categoryID+" title='Delete'><i class='fas fa-trash'></i></a></div>"])
			})
			table.draw(false);
		});
	}
	...
	$("#AddForm").submit(function(){
		...
		$.post( $("#AddForm").attr("action"), $("#AddForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Category successfully saved!");
			}
			else {
	...
	$("#UpdateForm").submit(function(){
		...
		$.post( $("#UpdateForm").attr("action"), $("#UpdateForm :input").serializeArray(), function(result){
			if (result == 'Success'){
				...
				successNotifNoload("Category successfully updated!");
			}
			else {
				errorNotifNoload(result);
			}
